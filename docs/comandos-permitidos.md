...
```
Lembramos que conforme explicitado no tópico [configurando ambiente local](ambiente-local.md), podemos suprimir os modificadores `-t` e `-p 9999`. Fica a critério do usuário.
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br postgres:expose teste
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br postgres:expose teste 9000
```
- `postgres:unexpose`
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br postgres:unexpose teste
```
- `postgres:export`
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br postgres:export [DB_NAME] > [DB_NAME].dump
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br postgres:import [DB_NAME] < [DB_NAME].dump
```
- `postgres:destroy`
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br postgres:destroy [NOME_DB]
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br postgres:connect [NOME_DB]
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br postgres:info [NOME_DB]
```
...
$ ssh -t -p 9999 dokku@app.ic.ufba.br run teste ls -lah
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br logs teste
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br urls teste
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br apps:destroy teste
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br postgres:destroy teste_db
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br config:show teste
```
Saída do comando:
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br config:set <NOME_DA_APLICAÇÃO> VAR1="valor\ 1" VAR2="valor\ 2"
```
Exemplo de execução em base64:

```
$ ssh -t -p 9999 dokku@app.ic.ufba.br config:set --encoded <NOME_DA_APLICAÇÃO> KEY="$(base64 ~/.ssh/id_rsa)
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br enter teste web echo olá 
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br ps:inspect <NOME_DA_APLICAÇÃO> 
```
- `ps:report`
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br ps:report <NOME_DA_APLICAÇÃO> 
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br ps:start <NOME_DA_APLICAÇÃO> 
```
- `ps:stop`
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br ps:stop <NOME_DA_APLICAÇÃO> 
```
- `ps:restart`
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br ps:restart <NOME_DA_APLICAÇÃO> 
```
- `ps:rebuild`
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br ps:rebuild <NOME_DA_APLICAÇÃO> 
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br cron:list <NOME_DA_APLICAÇÃO> 
```
- `cron:report <APP>`
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br cron:report <NOME_DA_APLICAÇÃO> 
```
