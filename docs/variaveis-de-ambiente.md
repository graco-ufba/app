...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br config:set <NOME_DO_APP> KEY="VALORES\ COM\ ESPAÇOS"
```
ou ainda:

```
ssh -t -p 9999 dokku@app.ic.ufba.br config:set sisfila VAR1="valor\ 1" VAR2="valor\ 2"
```
...
```
$ ssh -t -p 9999 dokku@app.ic.ufba.br config:set --encoded <NOME_DO_APP> KEY="$(base64 ~/.ssh/id_rsa)"
```
...
Obs.2: Lembramos que é possível suprimir os modificadores `-t` e `-p 9999` conforme indicado no tópico [configurando ambiente local](ambiente-local.md)
